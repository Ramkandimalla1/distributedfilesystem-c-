...
void handle_upload_command(int client_socket, char *command) {
    // Extract filename, destination path, and file content from the command
    char *filename = strtok(command + 7, " ");
    char *destination_path = strtok(NULL, " ");
    char *file_content = strtok(NULL, "");

    // Log the action of file creation
    ...
    if (fd < 0) {
        ...
        // Write the file content received from the command to the file
...
void handle_dfile_command(int client_socket, char *command) {
    ...
    if (fd < 0) {
        ...
        perror("File open failed");
        // Notify the client that file fetch failed
        send(client_socket, "Failed to fetch file", 20, 0);
    } else {
...
void handle_dtar_command(int client_socket, char *command) {
    ...
    // Buffer to hold the generated tar file name
    char tar_name[BUFFER_SIZE];
    // Path where the tar file will be created
    const char *dir_path = "~/spdf";

    // Format the tar file name using the HOME environment variable
    snprintf(tar_name, sizeof(tar_name), "%s/spdf/pdf.tar", getenv("HOME"));
    printf("%s \n", tar_name);

    // Create a tar file of specified file type in the given directory
    create_tar(tar_name, dir_path, filetype);
